...
- 6、设置vip, 设置推荐好友,设置超级会员 的url   ❌
   ...
   List<PerfConfigEntity> vipUrlList = configEntitiesMap.get(SettingEnum.SUPER_MEMBER_SETTING.getCode());
           if (!CollectionUtils.isEmpty(vipUrlList)) {
               userConfigVO.setVipUrl(vipUrlList.get(0).getSettingValue());
           }
   ```
   ...
   ```java
      // 设置推荐好友 url
           ...
           if (!CollectionUtils.isEmpty(rmUrlList)) {
               userConfigVO.setRmUrl(rmUrlList.get(0).getSettingValue());
           }
   ```
...
![image-20240402232232079](https://cdncode.oss-cn-beijing.aliyuncs.com/test/202404022322266.png)
